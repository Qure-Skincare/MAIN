...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function __handlerPlanBlock(e) {
        ...
        const ig_price = parseFloat(window.igData?.price?.getPriceByVariantId(product_variant_id)) || null;
        const ig_compare_at_price = parseFloat(window.igData?.price?.getComparePriceByVariantId(product_variant_id)) || null;
        ...
        if (totalPrice) {
            ...
            if (regularPrice) {
                ...
                if (regEl) regularPrice.textContent = ig_compare_at_price? currencySymbol + ig_compare_at_price : regEl.textContent;
            }

            if (salePrice) {
                const saleEl = this.querySelector(".sale_price");
                if (saleEl) salePrice.textContent = ig_price? currencySymbol + ig_price : saleEl.textContent.trim();
            }
        }
        ...
        document.querySelectorAll('.' + __section + " .pay_today").forEach(el => {
            el.textContent = ig_price? currencySymbol + (ig_price/4) : this.getAttribute("data-pay");
        });
